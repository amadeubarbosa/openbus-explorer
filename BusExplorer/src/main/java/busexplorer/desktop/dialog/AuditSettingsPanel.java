...
public class AuditSettingsPanel {
  ...
  public static AuditSettingsPanel create(Window parentWindow) {
    ...
    JPanel settingsPanel = new JPanel(new MigLayout("wrap 2, fill", "[grow][]", "[][][]"));

    settingsPanel.add(new JLabel(getString("enabled")), "grow");
    final JCheckBox auditEnabled = new JCheckBox();
    auditEnabled.setSelected(false);
    auditEnabled.setToolTipText(getString("enabled.tooltip"));
    settingsPanel.add(auditEnabled, "grow");

    settingsPanel.add(new JLabel(getString("url")), "grow");
    ...
    settingsPanel.add(new JScrollPane(auditServiceUrl), "grow");

    settingsPanel.add(new JLabel(getString("proxy")), "grow");
    final JTextArea httpProxy = new JTextArea(3, 35);
    httpProxy.setLineWrap(true);
    httpProxy.setToolTipText(getString("proxy.tooltip"));
    settingsPanel.add(new JScrollPane(httpProxy), "grow");

    settingsPanel.add(new JLabel(getString("credentials")), "grow");
    final JPasswordField httpCredentials = new JPasswordField();
    httpCredentials.setToolTipText(getString("credentials.tooltip"));
    settingsPanel.add(httpCredentials, "grow");

    settingsPanel.add(new JLabel(getString("fifolimit")), "grow");
    final JSpinner fifoLimit = new JSpinner(new SpinnerNumberModel(100000, 1, Integer.MAX_VALUE, 1));
    fifoLimit.setToolTipText(getString("fifolimit.tooltip"));
    settingsPanel.add(fifoLimit, "grow");

    settingsPanel.add(new JLabel(getString("fifolength")), "grow");
    ...
    refreshFifoLength.addActionListener(event -> {
      new BusExplorerTask<Integer>(ExceptionContext.BusCore) {
    ...
    settingsPanel.add(fifoLengthPanel, "growx");

    settingsPanel.add(new JLabel(getString("discard")), "grow");
    final JCheckBox discardOnExit = new JCheckBox();
    discardOnExit.setSelected(false);
    discardOnExit.setToolTipText(getString("discard.tooltip"));
    settingsPanel.add(discardOnExit, "grow");

    settingsPanel.add(new JLabel(getString("tasks")), "grow");
    ...
    settingsPanel.add(concurrentTasks, "grow");

    settingsPanel.add(new JLabel(getString("retrytimeout")), "grow");
    ...
    settingsPanel.add(retryTimeout, "grow");

    settingsPanel.add(new JLabel(getString("environment")), "grow");
    ...
    settingsPanel.add(new JScrollPane(auditEnvironment), "grow");

    settingsPanel.add(new JLabel(getString("application")), "grow");
    ...
    settingsPanel.add(auditApplication, "grow");
    ...
    result.updateTask = new BusExplorerTask<Void>(ExceptionContext.BusCore) {
      ...
      protected void doPerformTask() throws Exception {
        ...
        audit.setAuditServiceURL(auditServiceUrl.getText());
        audit.setAuditHttpProxy(httpProxy.getText());
        ...
        audit.setAuditEventTemplate(ENVIRONMENT, auditEnvironment.getText());
        audit.setAuditEventTemplate(APPLICATION, auditApplication.getText());
        ...
        if (audit.getAuditEnabled() == false) {
    ...
    applyButton.addActionListener(actionEvent -> result.updateTask
      .execute(parentWindow, Language.get(MainDialog.class, "conf.apply.waiting.title"),
        Language.get(MainDialog.class, "conf.apply.waiting.msg")));
    cancelButton.addActionListener(actionEvent -> result.retrieveTask
      .execute(parentWindow, Language.get(MainDialog.class, "conf.apply.waiting.title"),
        Language.get(MainDialog.class, "conf.apply.waiting.msg")));
